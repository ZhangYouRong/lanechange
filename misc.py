...
def distance_point_to_line(waypoint, vehicle_transform):
    x2 = vehicle_transform.location.x
    y2 = vehicle_transform.location.y
    t = waypoint.transform
    x1 = t.location.x
    y1 = t.location.y
    yaw = t.rotation.yaw
    k=math.tan(math.radians(yaw))
    d_lateral = (k*(x2-x1)+y1-y2)/math.sqrt(k**2+1)
    return d_lateral
