...
class VehiclePIDController():
    ...
    def __init__(self, vehicle, args_lateral=None, args_longitudinal=None):
        ...
        """
        if not args_lateral:
            args_lateral = {'K_P': 1.0, 'K_D': 0.0, 'K_I': 0.0, 'K': 10/540, 'Lookahead_Distance': 0.4}
...
class PIDLongitudinalController():
    ...
    def _pid_control(self, target_speed, current_speed):
        ...
        # 输出throttle 限幅化调整至throttle的范围0-1
...
class PIDLateralController():
    ...
    def __init__(self, vehicle, K_P=1.0, K_D=0.0, K_I=0.0, dt=0., K=10/540, Lookahead_Distance=0.4):
        """
        :param vehicle: actor to apply to local planner logic onto
        :param K_P: Proportional term
        :param K_D: Differential term
    ...
    def run_step(self, waypoint):
        ...
        """
        return self._lateral_control_distance(waypoint, self._vehicle.get_transform())

    def _pid_control_angle(self, waypoint, vehicle_transform):
        """
        ...
        w_vec = np.array([waypoint.transform.location.x-
                          ...
                          v_begin.y, 0.0])
        # 求v_vec和w_vec的夹角 并调整至[-1, 1]
        _dot = math.acos(np.clip(np.dot(w_vec, v_vec)/
                                 (np.linalg.norm(w_vec)*np.linalg.norm(v_vec)), -1.0, 1.0))

        _cross = np.cross(v_vec, w_vec)
        if _cross[2] < 0:  # 两个向量夹角大于180度，从v_vec算起。为了调整正负号
            _dot *= -1.0
    ...
    def _lateral_control_distance(self, waypoint, vehicle_transform):
        """
        Estimate the steering angle of the vehicle based on the PID equations
        ...
        :return: steering control in the range [-1, 1]
        """
        d_lateral = distance_point_to_line(waypoint, vehicle_transform)
        delta_fi = (vehicle_transform.rotation.yaw-waypoint.transform.rotation.yaw)%360
        if delta_fi > 180:
            delta_fi = delta_fi-360
